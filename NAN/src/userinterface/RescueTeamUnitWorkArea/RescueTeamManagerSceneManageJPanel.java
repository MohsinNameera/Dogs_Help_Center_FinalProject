...
public class RescueTeamManagerSceneManageJPanel extends javax.swing.JPanel {
    ...
    public RescueTeamManagerSceneManageJPanel(JPanel userProcessContainer, Enterprise enterprise, EcoSystem business, Organization organization) {
        initComponents();
    ...
    private void initComponents() {
        ...
        tblManageAssigned.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Report Id", "Report Name ", "Sender", "Assigned Rescue Team Admin", "Zip Code", "No of Dogs", "Status", "Creatd Date", " Message"
            }
        ) {
        ...
        jLabel2.setText("Rescue Team Admin");
        ...
        add(jButton2, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 450, -1, -1));

        jLabel3.setFont(new java.awt.Font("SansSerif", 1, 18)); // NOI18N
        jLabel3.setForeground(new java.awt.Color(25, 56, 82));
        jLabel3.setText("MANAGE  REPORT");
        add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(450, 70, -1, -1));
    }// </editor-fold>//GEN-END:initComponents

    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        VolunteerReportRequest scene = (VolunteerReportRequest) siteNameComboBox.getSelectedItem();
        Employee employee = (Employee) sceneManagerCombo.getSelectedItem();
        if(scene == null || employee == null){
            JOptionPane.showMessageDialog(null,"Select a Report and a Rescue Team manager");
        } else{
            String msg = JOptionPane.showInputDialog("Additional Message");
        
        scene.setSceneManager(employee);
        scene.setMessage(msg);
        scene.setStatus("Assigned Rescue verifying Team Admin");
        ...
        JOptionPane.showMessageDialog(null, "Rescue Verifying admin is assigned successfully");
        populateTable();
        populateSiteNameCombo();
        }
    ...
    private void populateTable() {
        ...
        for (WorkRequest wr : enterprise.getWorkQueue().getWorkRequestList()) {
            if (wr instanceof VolunteerReportRequest) {
                ...
                row[0] = ((VolunteerReportRequest) wr);
                row[1] = ((VolunteerReportRequest) wr).getSceneName();
                row[2] = ((VolunteerReportRequest) wr).getSender().getEmployee().getName() == null ? "" : ((VolunteerReportRequest) wr).getSender().getEmployee().getName();
                row[3] = ((VolunteerReportRequest) wr).getSceneManager() == null ? "" : ((VolunteerReportRequest) wr).getSceneManager().getName();
                row[4] = ((VolunteerReportRequest) wr).getSceneZipcode();
                row[5] = ((VolunteerReportRequest) wr).getNoOfVictims();
                row[6] = ((VolunteerReportRequest) wr).getStatus();
                row[7] = ((VolunteerReportRequest) wr).getRequestDate();
                row[8] = ((VolunteerReportRequest) wr).getMessage();
                model.addRow(row);
            }
        }
    ...
    private void populateSiteNameCombo() {
        ...
        for (WorkRequest wr : enterprise.getWorkQueue().getWorkRequestList()) {
            if(wr instanceof VolunteerReportRequest && ((VolunteerReportRequest) wr).getSceneManager() == null) {
                siteNameComboBox.addItem(((VolunteerReportRequest) wr));
            }
    ...
    private void populateSiteManagerCombo() {
        ...
        for(Organization o : enterprise.getOrganizationDirectory().getOrganizationList()) {
            for (UserAccount u : o.getUserAccountDirectory().getUserAccountList()) {
                if (u.getRole() instanceof ReporterAdmin) {
