...
public class VolunteerAdminReportManageJPanel extends javax.swing.JPanel {

    /**
     * Creates new form VolunteerAdminReportManageJPanel
     */
    ...
    Network network;
    EcoSystem business;
    RescueAdminReportRequest selectedSite;
            
    public VolunteerAdminReportManageJPanel(JPanel userProcessContainer, UserAccount account, Enterprise enterprise, Network network, EcoSystem business, Organization organization) {
        initComponents();
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        //ReportingAdminSceneRequest scene = (RescueAdminReportRequest) siteNameComboBox.getSelectedItem();
        
        String sceneName = (String) siteNameComboBox.getSelectedItem();        
        for (WorkRequest wr : organization.getWorkQueue().getWorkRequestList()) {
            if(wr instanceof RescueAdminReportRequest && ((RescueAdminReportRequest) wr).getSceneManager() ==  null) {
                if(sceneName.equals(((RescueAdminReportRequest) wr).getSceneName()))
                selectedSite = (RescueAdminReportRequest) wr;
            }            
        }
        RescueAdminReportRequest scene = selectedSite;
        
        Employee employee = (Employee) sceneManagerCombo.getSelectedItem();
        
        if("".equals(sceneName)) { 
            JOptionPane.showMessageDialog(null, "Name is not available for assignment");
            ...
            for (WorkRequest wr : empUserAccount.getWorkQueue().getWorkRequestList()) {            
                if(wr instanceof RescueAdminReportRequest) {
                    if(!((RescueAdminReportRequest) wr).getStatus().equals("Resolved")) {
                        isResolved = false;
                        break;
                    }
                }            
            ...
            if(isResolved) {
                ...
                scene.setStatus("Volunteer Manager Assigned");
                ...
                JOptionPane.showMessageDialog(null, "This manager is already assigned to an active Report. Please select a different manager.");
            }
        }
    ...
    private void populateTable() {
        ...
        for (WorkRequest wr : organization.getWorkQueue().getWorkRequestList()) {
            if (wr instanceof RescueAdminReportRequest && ((RescueAdminReportRequest) wr).getSceneManager() != null) {
                ...
                row[2] = ((RescueAdminReportRequest) wr).getSceneManager();
                row[3] = ((RescueAdminReportRequest) wr).getSender();
                row[4] = ((RescueAdminReportRequest) wr).getStatus();
                row[5] = ((RescueAdminReportRequest) wr).getMessage();
                model.addRow(row);
            }
        }
    ...
    private void populateSiteNameCombo() {
        ...
        for (WorkRequest wr : organization.getWorkQueue().getWorkRequestList()) {
            if(wr instanceof RescueAdminReportRequest && ((RescueAdminReportRequest) wr).getSceneManager() ==  null) {
                siteNameComboBox.addItem(((RescueAdminReportRequest) wr).getSceneName());
            }            
        }
    }
