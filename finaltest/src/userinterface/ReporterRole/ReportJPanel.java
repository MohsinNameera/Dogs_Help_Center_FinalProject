...
public class ReportJPanel extends javax.swing.JPanel {
     ...
     */
    public ReportJPanel(JPanel userProcessContainer, UserAccount account, EcoSystem ecosystem) {
        initComponents();
        initListners();
        ...
        HelpCenterDirectory = ecosystem.getHelpCenterDirectory();
        menuTable = (DefaultTableModel) tblOrder.getModel();
        cartTable = (DefaultTableModel) tblCart.getModel();
        populateRestaurantsList(HelpCenterDirectory.getHelpCenterList());
        if (HelpCenterDirectory.getHelpCenterList().size() > 0) {
            index = 0;
            populateMenu();
        }
    ...
    public void populateRestaurantsList(ArrayList<HelpCenter> HelpCenterList) {
        for (HelpCenter HelpCenter : HelpCenterList) {
            restaurantComboBox.addItem(HelpCenter.getName());
        }
    }
    ...
    private void populateMenu() {
        ...
        HelpCenter HelpCenter = HelpCenterDirectory.getHelpCenterList().get(index);
        Menu menu = HelpCenter.getMenu();
    ...
    private boolean createOrder() {
        ...
        if (itemsWithQuantityList.isEmpty()) {
            ...
            if (Reporter != null) {
                orderWorkRequest.setReporter(Reporter);
            } else {
            ...
            HelpCenter HelpCenter = HelpCenterDirectory.getHelpCenterList().get(index);
            if (HelpCenter != null) {
                orderWorkRequest.setHelpCenter(HelpCenter);
            } else {
...
//    }
    ...
    private void requestTestJButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_requestTestJButtonActionPerformed
        if (createOrder()) {
            ...
            ReporterStatusJPanel orderStatusJPanel = new ReporterStatusJPanel(userProcessContainer, ecosystem, Reporter);
